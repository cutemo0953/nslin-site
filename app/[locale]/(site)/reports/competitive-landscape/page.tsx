...
export async function generateMetadata({
  ...
  return {
    title: isZh
      ? '無內胎氣嘴競品分析'
      : 'Tubeless Valve Competitive Landscape',
...
function ReportFooter({ slug, locale }: { slug: string; locale: string }) {
  ...
  // Staleness check: > 14 days since last update
  ...
  return (
    <footer className="mt-12 border-t border-metal-200 pt-6 text-sm text-metal-400 space-y-1">
      ...
      <p>
        {isZh ? '下次自動收集：每週一' : 'Next auto-collection: every Monday'}
      </p>
  ...
  return (
    <article className="py-8 lg:py-12">
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        ...
        />
        <ReportFooter slug="competitive-landscape" locale={locale} />
      </div>
    </article>
  );
}
